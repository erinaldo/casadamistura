...
namespace PdvStock.Utils
{
    public static class FileUpload
    {
        ...
        public static string FilesPath = HttpContext.Current.Server.MapPath("~\\Fotos" + DirSeparator);
        ...
        public static string UploadFoto(HttpPostedFileBase file, String fileName)
        {
            // Check if we have a file
            if (null == file) return "";
            ...
            // Set our full path for saving
            string path = FilesPath + DirSeparator + fileName + ".jpeg";
            // Save our file
            file.SaveAs(Path.GetFullPath(path));

            //Image image1 = Image.FromFile(Path.GetFullPath(path));
            //image1.Save(Path.GetFullPath(path).Replace(fileExt, ""), ImageFormat.Jpeg);
            // Return the filename
            return fileName;
        }


        public static void DeleteFoto(string fileName)
        {
