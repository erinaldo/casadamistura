...
namespace PDV
{
    public partial class Recebimento : Form
    {
        ...
        private void Recebimento_Load(object sender, EventArgs e)
        {
            ...
            dinheirotext.Text = ValorTotal.ToString("N");
        ...
        {
            string sQuery = null;
            sQuery = sQuery + string.Format("SELECT Id,Descricao,Descricao tipos FROM P_formaspgto ORDER BY Ordenacao");
        ...
        private void jogavalor()
        {
            ...
            {
                dataGridView1.Rows.Add(cbodebito.Text, dinheirotext.Text);
            }
        ...
        private void Finaliza_Impressora(bool fiscal)
        {
            ...
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                if (fiscal)
                {
                    ...
                    bool impresso = Sat(cpf);
                    vcodigofluxo = venda.CadastraFluxo(Pdv.lcaixa.Text, cpf, impresso);
                }
                else
                {
                ...
                Pdv.valorunitario.Text = "0";
                Pdv.subtotal.Text = "0";
                Pdv.ldescricaoproduto.Text = "";
                Pdv.linformacoes.Text = "";
                Pdv.carregaimagemproduto.Image = null;
                Pdv.carregaimagemproduto.InitialImage = null;
                this.Close();
            }
            else
            {
        ...
        private void Impressora_nao_fiscal()
        {
            ...
            iRetorno = MP2032.FormataTX("     CASA DA MISTURA                  " + "\r\n", 3, 0, 0, 1, 1);
            iRetorno = MP2032.FormataTX("LE DELICE COMERCIAL DE ALIMENTOS LDTA" + "\r\n", 2, 0, 0, 0, 0);
            iRetorno = MP2032.FormataTX("Rua São Teodoro,825 Vila Carmosina" + "\r\n", 2, 0, 0, 0, 0);
            iRetorno = MP2032.FormataTX("SÃO PAULO/SP" + "\r\n", 2, 0, 0, 0, 0);
            iRetorno = MP2032.FormataTX("CNPJ: 72947831/0001-86" + "\r\n", 2, 0, 0, 0, 0);
            iRetorno = MP2032.FormataTX("IE:113916339119" + "\r\n", 2, 0, 0, 0, 0);
            ...
            foreach (DataGridViewRow linha in Pdv.dataGridView1.Rows)
            {
                ...
                Nomeproduto = Nomeproduto.PadRight(30, ' ');
                PesoProduto = Pdv.dataGridView1[7, linha.Index].Value.ToString() + " g";
                PesoProduto = PesoProduto.PadRight(10, ' ');
        ...
        private void Impressora_nao_fiscal_NOVO()
        {
        ...
        private bool Sat(string CpfCnpj)
        {
            bool deucerto = true;
            //int ACK, ST1, ST2, ST3;
            #region ABRECUPOM
            ...
            double TotalImposto = 0;
            StreamWriter grava = new StreamWriter(Application.StartupPath + @"\SAT" + DateTime.Now.ToShortDateString().Replace('/', '_') + "_" + DateTime.Now.ToLongTimeString().Replace(':', '_') + ".log", true);
            grava.WriteLine(DateTime.Now.ToString() + " : IMPRINDO CUPOM - " + CpfCnpj);
            Pdv.linformacoes.Text = "IMPRINDO CUPOM";
            iRetorno = BemaFI32.Bematech_FI_AbreCupomMFD(CpfCnpj, "", "");
            grava.WriteLine(DateTime.Now.ToString() + " : Bematech_FI_AbreCupomMFD - " + iRetorno);
            ...
            if (ST3 == 7)
            {
                grava.WriteLine(DateTime.Now.ToString() + " : Cupom Fiscal Aberto- " + iRetorno);
                Pdv.linformacoes.Text = "Cupom Fiscal Aberto!";
                deucerto = true;
            }
            ...
            foreach (DataGridViewRow linhas in dataGridView1.Rows)
            {
                //conex.GeraErro("TESTE FORMA PGTO", "FORMA:" + dataGridView1[0, linhas.Index].Value.ToString(), DateTime.Now.ToString());
                IRetorno = BemaFI32ImpressoraFiscal.Bematech_FI_EfetuaFormaPagamento(dataGridView1[0, linhas.Index].Value.ToString(), dataGridView1[1, linhas.Index].Value.ToString());
            }
            BemaFI32.Analisa_iRetorno(iRetorno);
            ...
            string assinaturaSW = "";
        ...
        {
            if (cbodebito.Text.Contains("CARTÃO CREDITO"))
            {
        ...
        {
            //if(cbodebito.Text.Contains("CARTÃO CREDITO") && Convert.ToInt32((parcelas.Text==null?"0":(parcelas.Text==""?"0": parcelas.Text))) > 0)
            //{
                Finaliza_Impressora(true);
            //}
            //else
            //{
            //    MessageBox.Show("PARA PAGAMENTOS COM CARTÃO DE CRÉDITO É NECESSÁRIO ESCOLHER A QUANTIDADE DE PARCELAS", "ATENÇÃO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //}
